...
for _ in range(1, NumOfSamples):
    ...
    qParticles = computeNewParticlesPose(
        qParticles, np.dot(actuatorVarNoise, np.random.rand(3, 1)) + curSpeedEst, Ts
    )

    # corection step
    # get simulated robot sensor value
    is_detected, detObjInd, objPosRobotLocal = getSimulatedRobotSensorValue(
        qRobotRealPose, objPos
    )
    ...
    if is_detected:
        ...
        particlesWeights = computeParticleWeights(
            innovMat, computeCovarianceMat(sensorVariance)
        )
